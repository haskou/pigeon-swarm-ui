...
export function CallScreenShareStage({
  ...
  return (
    <div
      className="group relative min-h-0 flex-1 overflow-hidden rounded-[1.35rem] border border-white/10 bg-black text-left shadow-2xl shadow-black/45"
    ...
    >
      ...
      />
      <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-center justify-between bg-gradient-to-b from-black/75 to-transparent px-4 py-3">
        <div className="min-w-0">
          <p className="truncate text-sm font-black text-white">
            {participantName}
          </p>
          <p className="text-xs font-bold text-emerald-200/85">
            {copy.calls.screen}
          </p>
        </div>
        <span className="rounded-full bg-rose-500 px-2.5 py-1 text-xs font-black text-white shadow-lg shadow-black/30">
          {copy.calls.live}
        </span>
      </div>
