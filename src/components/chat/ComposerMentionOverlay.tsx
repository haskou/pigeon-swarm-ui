...
export function ComposerMentionOverlay({
  ...
  return (
    <div
      aria-hidden="true"
      className="pointer-events-none absolute inset-0 overflow-hidden px-2 py-2 text-sm leading-5 tracking-normal text-white [font:inherit]"
    >
...
function highlightMentionTokens(value: string, mentionTokens: string[]) {
  ...
  while ((match = pattern.exec(value))) {
    if (match.index > cursor) {
      parts.push({ highlighted: false, text: value.slice(cursor, match.index) });
    }
